    ...
    try:
        ...
        for mrr in mrrs:
            ...
            try:
                ...
                error("mrrid({0}) is not integer.".format(
                    m.find('mrrid').text))

            if mrrid < 1000 or mrrid > 65535:
                error("mrrid({0}) not valid. (1000 <= mrrid <= 65535))".format(
                    str(mrrid)))
            ...
            try:
                ...
                error("status({0}) is not integer.".format(
                    r.find('status').text))
            ...
            # print("}...OK!")
            mrrdata.update(
                {mrrid: {"trigger": trigger_data, "response": response_data}})
